...
int main(int argc, char* argv[]){
    ...
    // For Core Affinity
    // Needed for setting of core affinity of threads
    pthread_attr_t attr;
    cpu_set_t cpus;
    ...
    for (int i = 0; i < threadSize; i++) {
        // Threads
        setCPU = i % core_number;
        CPU_ZERO(&cpus);
        CPU_SET(setCPU, &cpus);

        // Sets core affinity of the thread
		// sched_setaffinity(threads[i], sizeof(cpus), &cpus);
		printf("core set is: %d\n", setCPU);
        pthread_attr_setaffinity_np(&attr, sizeof(cpu_set_t), &cpus);
        pthread_create(&threads[i], &attr, thread_pearson, (void*) &params[i]);
    }
